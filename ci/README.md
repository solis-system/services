...
## Usage dans Woodpecker CI

L'image est utilisée dans les pipelines `.woodpecker.yml` des repos `api` et `lolapp` :
...
| `CI_COMMIT_BRANCH` | Branche Git courante (fournie par Woodpecker)| `main`, `develop`              |
...
Pour tester l'image localement sans Woodpecker :
...
3. Les prochaines exécutions Woodpecker utiliseront automatiquement la nouvelle version (grâce à `pull: true`)
