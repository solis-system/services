...
echo "🔀 Current branch: ${CI_COMMIT_BRANCH:-unknown}"
...
if [ -z "$CI_COMMIT_BRANCH" ]; then
  echo "❌ ERROR: CI_COMMIT_BRANCH is required"
  exit 1
fi
...
if [[ "$CI_COMMIT_BRANCH" == "main" ]]; then
  ...
  echo "🏷️ Environment: Branch ($CI_COMMIT_BRANCH)"
  DOCKER_TAGS+=("$CI_COMMIT_BRANCH")
fi
